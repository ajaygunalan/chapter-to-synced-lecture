...
idea is for, run it on a small example by hand, and tell where it breaks.

## The one rule

**The book holds the truth. The lecture's only job is to carry that truth
into the listener's head, by whatever means works best** — the book's example
or a made-up one, code, a picture, a story, a joke — chosen freely and
creatively for each moment, and never at the cost of saying something false.
...
lecture stops until the listener chooses to go on. A likely wrong guess is
the best reason to stop — the answer then opens with the trap — but having
to think is reason enough. The content decides how many stops there are; a
long stretch with nothing to predict is a sign the material has been told
rather than shown.
...
already obvious is not marked. The formal statement appears after the
example has shown why it must be true.
